...
export async function generateChatTitle(userMessage: string): Promise<string> {
  try {
    ...
    try {
      ...
      logger.error(
        `Failed to get API key for title generation (${provider} ${model}):`,
        error
      );
...
> {
  ...
  try {
    ...
    const results = await db
      ...
      .from(docsEmbeddings)
      .orderBy(
        sql`${docsEmbeddings.embedding} <=> ${JSON.stringify(queryEmbedding)}::vector`
      )
      .limit(topK);
    ...
    logger.info(
      `Found ${filteredResults.length} relevant documentation chunks`,
      {
  ...
  try {
    ...
    if (searchResults.length === 0) {
      const fallbackResponse =
        "I couldn't find any relevant documentation for your question. Please try rephrasing your query or check if you're asking about a feature that exists in Visual Workflow AI.";
      return {
...
Content: ${result.content}`;
    ...
    if (conversationHistory.length > 0) {
      ...
      conversationHistory
        .slice(-config.general.maxConversationHistory)
        .forEach((msg) => {
    ...
    const systemPrompt = `You are a helpful assistant that answers questions about Visual Workflow AI documentation. You are having a conversation with the user, so refer to the conversation history when relevant.
...
${context}`;
    ...
    if ("stream" in response && "execution" in response) {
      ...
      throw new Error(
        "Unexpected streaming execution response when non-streaming was requested"
      );
    }
...
export async function generateChatResponse(
  ...
  try {
    ...
    try {
      ...
      logger.error(
        `Failed to get API key for chat (${provider} ${model}):`,
        error
      );
      throw new Error(
    ...
    const tools: ProviderToolConfig[] = [
      ...
      },
      {
        ...
        description:
          'Get the user\'s current workflow - this shows ONLY the blocks they have actually built and configured in their specific workflow, not general Visual Workflow AI capabilities. Use this when the user asks about "my workflow", "this workflow", wants to know what blocks they currently have, OR when they ask "How do I..." questions about their workflow so you can give specific, actionable advice based on their actual setup.',
        params: {},
        parameters: {
          type: "object",
          properties: {
...
export async function createChat(
  ...
  try {
    ...
    return {
      ...
      messageCount: Array.isArray(newChat.messages)
...
export async function updateChat(
  ...
  try {
    ...
    return {
      ...
      messageCount: Array.isArray(updatedChat.messages)
...
export async function sendMessage(request: SendMessageRequest): Promise<{
  ...
  const { message, chatId, workflowId, createNewChat, stream, userId } =
    request;
