    ...
    <AlertDialog open={isOpen} onOpenChange={onClose}>
      <AlertDialogContent className="z-[100]">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Are you sure you want to delete this API deployment? This action
            cannot be undone.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
...
export function NotificationList() {
  ...
  const visibleCount = activeWorkflowId
    ? getVisibleNotificationCount(activeWorkflowId)
    : 0;
      ...
      >
        {visibleNotifications.map((notification) => (
          <div
            ...
            className={cn(
              "notification-container",
              animatingIds.has(notification.id) &&
                "animate-notification-slide-up"
            )}
          >
            <NotificationAlert
...
export function NotificationAlert({
  ...
  const formatCurlCommand = (command: string, apiKey: string) => {
    ...
    const sanitizedCommand = command.replace(apiKey, "FLOW_API_KEY");
            ...
            <div className="mx-4 flex-1 space-y-2 pt-[3.5px] pr-4">
              ...
              <AlertDescription className="space-y-4">
                ...
                {options?.sections?.map((section, index) => {
                  ...
                  const apiKey =
                    options.sections?.find((s) => s.label === "x-api-key")
                      ?.content || "";
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            if (!response.ok)
                              throw new Error("Failed to redeploy workflow");
                            ...
                            logger.error("Error redeploying workflow:", {
                              error,
                            });
                          }
                        }}
