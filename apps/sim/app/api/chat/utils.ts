...
declare global {
  var __chatStreamProcessingTasks:
...
export async function validateChatAuth(
  ...
  if (authType === "password") {
    ...
    try {
      ...
      if (!deployment.password) {
        logger.error(
          `[${requestId}] No password set for password-protected chat: ${deployment.id}`
        );
        return {
          authorized: false,
  ...
  if (authType === "email") {
    ...
    try {
      ...
      if (
        domain &&
        allowedEmails.some((allowed: string) => allowed === `@${domain}`)
      ) {
...
export async function executeWorkflowForChat(
  ...
  if (workflowResult.length === 0 || !workflowResult[0].isDeployed) {
    logger.warn(
      `[${requestId}] Workflow not found or not deployed: ${workflowId}`
    );
    ...
    async start(controller) {
        ...
        try {
          while (true) {
            ...
            if (done) {
              controller.enqueue(
                encoder.encode(
                  `data: ${JSON.stringify({ blockId, event: "end" })}\n\n`
                )
              );
              break;
            }
      ...
      const executor = new Executor({
        ...
        contextExtensions: {
          stream: true,
          selectedOutputIds:
            selectedOutputIds.length > 0 ? selectedOutputIds : outputBlockIds,
      ...
      if (result && "success" in result) {
        ...
        if (result.logs) {
          ...
          logger.info(
            `[CHAT-API] Processed ${processedCount} blocks for streaming tokenization`
          );
        }
        ...
        const executionId = uuidv4();
        logger.debug(
          `Generated execution ID for deployed chat: ${executionId}`
        );
        ...
        if (result.success) {
          try {
            ...
            logger.debug(
              `Updated user stats for deployed chat: ${deployment.userId}`
            );
          } catch (error) {
            logger.error(
              `Failed to update user stats for deployed chat:`,
              error
            );
          }
        }
