...
const X_TESTIMONIALS = [
  ...
  {
    text: "One of the best products I've seen in the space, and the hustle and grind I've seen from @karabegemir and @typingwala is insane. Visual Workflow AI is positioned to build something game-changing, and there's no better team for the job.\n\nCongrats on the launch 🚀 🎊 great things ahead!",
  ...
  {
    text: "lfgg got access to @visualworkflowaiai via @zerodotemail 😎",
    username: "@nizzyabi",
    viewCount: "1,585",
  ...
  {
    text: "The use cases are endless. Great work @visualworkflowaiai",
...
const firstRowTestimonials = X_TESTIMONIALS.slice(
  0,
  Math.ceil(X_TESTIMONIALS.length / 2)
);
const secondRowTestimonials = X_TESTIMONIALS.slice(
  Math.ceil(X_TESTIMONIALS.length / 2)
);
          ...
          <Marquee className="flex w-full [--duration:40s]" pauseOnHover={true}>
            {firstRowTestimonials.map((card, index) => (
              <motion.div
                ...
                whileHover={{
                  scale: 1.02,
                  boxShadow: "0 8px 32px 0 rgba(80, 60, 120, 0.18)",
                }}
                transition={{ type: "spring", stiffness: 300, damping: 20 }}
                onClick={() =>
                  card.tweetUrl &&
                  window.open(card.tweetUrl, "_blank", "noopener,noreferrer")
                }
              >
