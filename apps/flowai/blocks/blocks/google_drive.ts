...
export const GoogleDriveBlock: BlockConfig<GoogleDriveResponse> = {
  ...
  docsLink: "https://docs.flowai-tau.vercel.app/tools/google_drive",
  category: "tools",
  bgColor: "#E0E0E0",
    ...
    config: {
      tool: (params) => {
        switch (params.operation) {
          ...
          default:
            throw new Error(
              `Invalid Google Drive operation: ${params.operation}`
            );
        }
      },
      params: (params) => {
        const { credential, folderId, folderSelector, mimeType, ...rest } =
          params;
