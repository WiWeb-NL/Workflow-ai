...
export const SlackBlock: BlockConfig<SlackResponse> = {
  ...
  docsLink: "https://docs.flowai-tau.vercel.app/tools/slack",
  category: "tools",
  bgColor: "#611f69",
  icon: SlackIcon,
  subBlocks: [
    ...
    },
    {
      ...
      layout: "full",
      options: [
        { label: "Visual Workflow AI Bot", id: "oauth" },
    ...
    config: {
      ...
      params: (params) => {
        ...
        if (authMethod === "bot_token") {
          if (!botToken) {
            throw new Error(
              "Bot token is required when using bot token authentication"
            );
          }
          ...
          if (!credential) {
            throw new Error(
              "Slack account credential is required when using Visual Workflow AI Bot"
            );
          }
