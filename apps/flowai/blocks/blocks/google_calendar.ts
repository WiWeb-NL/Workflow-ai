...
export const GoogleCalendarBlock: BlockConfig<GoogleCalendarResponse> = {
  ...
  docsLink: "https://docs.flowai-tau.vercel.app/tools/google-calendar",
  category: "tools",
  bgColor: "#E0E0E0",
    ...
    config: {
      tool: (params) => {
        switch (params.operation) {
          ...
          default:
            throw new Error(
              `Invalid Google Calendar operation: ${params.operation}`
            );
        }
      },
      params: (params) => {
        const { credential, operation, attendees, replaceExisting, ...rest } =
          params;
        ...
        if (
          ["create", "quick_add", "invite"].includes(operation) &&
