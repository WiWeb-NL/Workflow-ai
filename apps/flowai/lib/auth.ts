...
const resend = validResendAPIKEY
  ...
  : {
      emails: {
        send: async (...args: any[]) => {
          logger.info(
            "Email would have been sent in production. Details:",
            args
          );
      ...
      ? [
          stripe({
            ...
            subscription: {
              ...
              onSubscriptionComplete: async ({
                ...
                if (subscription.plan === "team") {
                  try {
                    ...
                    if (user.length > 0) {
                      ...
                      logger.info(
                        "Auto-created organization for team subscription",
                        {
                          organizationId: orgId,
                          userId: currentUser.id,
                          subscriptionId: subscription.id,
                    ...
                    logger.error(
                      "Failed to auto-create organization for team subscription",
                      {
                        subscriptionId: subscription.id,
                        referenceId: subscription.referenceId,
                        error,
                ...
                try {
                  const { syncUsageLimitsFromSubscription } = await import(
                    "./billing"
                  );
                  const { initializeBillingPeriod } = await import(
                    "./billing/core/billing-periods"
                  );

                  await syncUsageLimitsFromSubscription(
                    subscription.referenceId
                  );
                  logger.info(
                    "Usage limits synced after subscription creation",
                    {
                      referenceId: subscription.referenceId,
                    }
                  );
                  ...
                  if (subscription.plan !== "free") {
                    const stripeStart = new Date(
                      stripeSubscription.current_period_start * 1000
                    );
                    const stripeEnd = new Date(
                      stripeSubscription.current_period_end * 1000
                    );

                    await initializeBillingPeriod(
                      subscription.referenceId,
                      stripeStart,
                      stripeEnd
                    );
              ...
              onSubscriptionUpdate: async ({
                ...
                try {
                  ...
                  logger.error(
                    "Failed to sync usage limits after subscription update",
                    {
                      referenceId: subscription.referenceId,
                      error,
                    }
          ...
          organization({
            ...
            beforeInvite: async ({
              ...
              if (!teamSubscription) {
                throw new Error(
                  "No active team subscription for this organization"
                );
              }
