...
export async function sendEmail({
  ...
  try {
    ...
    if (emailType !== "transactional") {
      const unsubscribeType = emailType as
        | "marketing"
        | "updates"
        | "notifications";
...
export async function sendBatchEmails({
  ...
  try {
    ...
    for (let i = 0; i < batchEmails.length; i += BATCH_SIZE) {
      if (i > 0) {
        logger.info(
          `Rate limit protection: Waiting ${rateDelay}ms before sending next batch`
        );
      ...
      try {
        ...
        if (response.error) {
          ...
          if (Array.isArray(response.data)) {
            ...
            logger.info(
              "Resend batch API returned unexpected format, assuming success"
            );
        ...
        if (
          ...
          try {
            logger.info(
              `Retrying batch ${Math.floor(i / BATCH_SIZE) + 1} with longer delay`
            );
            ...
            if (retryResponse.error) {
              ...
              batch.forEach(() => {
                results.push({
                  ...
                  message:
                    ...
                    "Failed to send batch email after retry",
                });
              });
              ...
              logger.info(
                "Increasing delay between batches after rate limit hit"
              );
