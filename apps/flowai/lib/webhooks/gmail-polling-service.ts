  ...
  try {
    ...
    const enqueue = async (webhookData: (typeof activeWebhooks)[number]) => {
      ...
      try {
        ...
        if (!userId) {
          logger.error(
            `[${requestId}] No user ID found for webhook ${webhookId}`
          );
        ...
        if (!accessToken) {
          logger.error(
            `[${requestId}] Failed to get Gmail access token for webhook ${webhookId}`
          );
        ...
        const config =
          webhookData.providerConfig as unknown as GmailWebhookConfig;
        ...
        if (!emails || !emails.length) {
          ...
          logger.info(
            `[${requestId}] No new emails found for webhook ${webhookId}`
          );
          return { success: true, webhookId, status: "no_emails" };
        }

        logger.info(
          `[${requestId}] Found ${emails.length} new emails for webhook ${webhookId}`
        );
        ...
        logger.error(
          `[${requestId}] Error processing Gmail webhook ${webhookId}:`,
          error
        );
  ...
  try {
    ...
    if (useHistoryApi) {
      ...
      if (!historyResponse.ok) {
        ...
        logger.info(
          `[${requestId}] Falling back to search API after history API failure`
        );
      ...
      logger.info(
        `[${requestId}] Processing ${idsToFetch.length} emails from history API`
      );
      ...
      emails = emailResults
        .filter(
          (result): result is PromiseFulfilledResult<GmailEmail> =>
  ...
  try {
    ...
    if (config.lastCheckedTimestamp) {
      ...
      const minutesSinceLastCheck =
        (now.getTime() - lastCheckedTime.getTime()) / (60 * 1000);
      ...
      if (minutesSinceLastCheck < 60) {
        ...
        const bufferSeconds = Math.max(
          (config.pollingInterval || 2) * 60 * 2,
          180
        );

        // Calculate the cutoff time with buffer
        const cutoffTime = new Date(
          lastCheckedTime.getTime() - bufferSeconds * 1000
        );
        ...
        logger.debug(
          `[${requestId}] Using timestamp-based query with ${bufferSeconds}s buffer`
        );
      }
      // If last check was a while ago, use Gmail's relative time queries
        ...
        logger.debug(
          `[${requestId}] Using hour-based query: newer_than:${hours}h`
        );
      } else {
        // For very old last checks, limit to a reasonable time period (7 days max)
        const days =
          Math.min(Math.ceil(minutesSinceLastCheck / (24 * 60)), 7) + 1;
        timeConstraint = ` newer_than:${days}d`;
        logger.debug(
          `[${requestId}] Using day-based query: newer_than:${days}d`
        );
      }
    } else {
      ...
      logger.debug(
        `[${requestId}] No last check time, using default: newer_than:1d`
      );
    }
    ...
    const idsToFetch = searchData.messages.slice(
      0,
      config.maxEmailsPerPoll || 25
    );
    let latestHistoryId = config.historyId;
  ...
  for (const email of emails) {
    try {
      ...
      try {
        ...
        logger.warn(
          `[${requestId}] Redis check failed for ${email.id}, continuing`,
          err
        );
      }
...
async function updateWebhookLastChecked(
  ...
  const existingConfig =
    (await db.select().from(webhook).where(eq(webhook.id, webhookId)))[0]
      ?.providerConfig || {};
  await db
    .update(webhook)
    .set({
