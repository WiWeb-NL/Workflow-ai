...
export async function getUserUsageData(userId: string): Promise<UsageData> {
  try {
    ...
    const limit = Number.parseFloat(stats.currentUsageLimit ?? "5");
    const percentUsed =
    ...
    return {
      ...
      lastPeriodCost: Number.parseFloat(
        stats.lastPeriodCost?.toString() || "0"
      ),
    };
  } catch (error) {
 ...
 */
export async function getUserUsageLimitInfo(
  userId: string
): Promise<UsageLimitInfo> {
  try {
    ...
    return {
      currentLimit: Number.parseFloat(stats.currentUsageLimit ?? "5"),
      canEdit,
      minimumLimit,
  ...
  try {
    ...
    if (!canEdit) {
      if (subscription?.plan === "team") {
        return {
          success: false,
          error: "Only team owners and admins can edit usage limits",
        };
      }
      return {
        success: false,
        error: "Free plan users cannot edit usage limits",
      };
    }
...
export async function getUserUsageLimit(userId: string): Promise<number> {
  try {
    ...
    return Number.parseFloat(userStatsQuery[0].currentUsageLimit ?? "5");
  } catch (error) {
 ...
 */
export async function syncUsageLimitsFromSubscription(
  userId: string
): Promise<void> {
  try {
    const subscription = await getHighestPrioritySubscription(userId);
    ...
    if (currentUserStats.length === 0) {
      ...
      logger.info("Created usage stats with synced limit", {
        userId,
        limit: defaultLimit,
      });
      return;
    ...
    const currentLimit = Number.parseFloat(
      currentStats.currentUsageLimit ?? "5"
    );
 ...
 */
export async function calculateBillingProjection(
  userId: string
): Promise<BillingData> {
  try {
    const usageData = await getUserUsageData(userId);
    ...
    const dailyRate =
      daysElapsed > 0 ? usageData.currentUsage / daysElapsed : 0;
