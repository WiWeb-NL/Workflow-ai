  ...
  try {
    ...
    for (const log of result.logs || []) {
      ...
      if (
        ...
        logger.debug(
          "Processing LLM-based block output for tool calls and cost tracking",
          {
            blockId: log.blockId,
        ...
        if (
          ...
          (!log.output.tokens?.completion ||
            log.output.tokens.completion === 0) &&
          ...
          if (log.output.providerTiming) {
            logger.debug(
              "Processing streaming response without tool calls for token extraction",
              {
                blockId: log.blockId,
            ...
            if (contentLength > 0) {
              ...
              logger.debug(
                "Updated token information for streaming no-tool response",
                {
                  blockId: log.blockId,
                  contentLength,
                  estimatedCompletionTokens,
        ...
        if (log.output?.cost) {
          ...
          if (output.cost.total) {
            ...
            if (output.model) {
              ...
              if (
                !primaryModel ||
                modelCounts[output.model] > modelCounts[primaryModel]
              ) {
        ...
        // Case 2: toolCalls with a list array (as seen in the screenshot)
          ...
          log.output.toolCalls &&
          Array.isArray(log.output.toolCalls.list)
        ) {
          // Log raw timing data for debugging
        ...
        // Case 4: Look in executionData.output for streaming responses
          ...
          const list = Array.isArray(toolCallsObj)
            ? toolCallsObj
            : toolCallsObj.list || [];
        ...
        // Verbose output debugging as a fallback
          ...
          logger.debug(
            "Could not find tool calls in standard formats, output data:",
            {
        ...
        if (toolCallData.length > 0) {
          ...
          logger.debug("Added tool calls to metadata", {
            ...
            existingMetadata: Object.keys(metadata).filter(
              (k) => k !== "toolCalls"
            ),
          });
        }
    ...
    if (totalCost > 0) {
      ...
      console.log(
        `[DEBUG] PersistExecutionLogs: userId=${userId}, result.success=${result.success}, workflowId=${workflowId}, executionId=${executionId}`
      );

      if (userId) {
        console.log(`[DEBUG] UserId found, entering token charging logic`);
        try {
          // Charge FlowAI tokens for successful executions
          if (result.success) {
            console.log(
              `[DEBUG] Execution was successful, attempting to charge FlowAI tokens`
            );
            logger.debug(`Attempting to charge FlowAI tokens for execution`, {
              userId,
              executionId,
              workflowId,
            });
            ...
            console.log(
              `[DEBUG] FlowAI token charging result: ${tokenCharged} for execution ${executionId}`
            );

            if (!tokenCharged) {
              console.log(
                `[DEBUG] Token charging failed for execution ${executionId}`
              );
              logger.warn(
              ...
              console.log(
                `[DEBUG] Token charging successful for execution ${executionId}`
              );
              logger.info(
                `Successfully charged FlowAI tokens for execution ${executionId}`,
                {
                  userId,
                  workflowId,
                  tokensCharged: WORKFLOW_EXECUTION_COST,
                }
              );
            }
          }
          ...
          if (userStatsRecords.length === 0) {
            await db.insert(userStats).values({
              ...
              // FlowAI token tracking initialized in the service
              lastActive: new Date(),
            });
          } else {
            await db
              ...
              .set({
                ...
                lastActive: new Date(),
                // FlowAI token updates handled by the service
              })
              .where(eq(userStats.userId, userId));
          }
        } catch (error) {
          console.log(
            `[DEBUG] Error in FlowAI token charging or user stats update:`,
            error
          );
          logger.error("Error upserting user stats:", error);
        }
      } else {
        console.log(`[DEBUG] No userId found, skipping FlowAI token charging`);
      }
    }
    ...
    await persistLog({
      ...
      message: result.success
        ? successMessage
        : `${errorPrefix} execution failed: ${result.error}`,
...
function getToolCallTimings(
  ...
  const result = toolCalls.map((toolCall, index) => {
    ...
    if (!enhancedToolCall.endTime) {
      const endTimestamp =
        new Date(enhancedToolCall.startTime).getTime() +
...
function extractDuration(toolCall: any): number {
  ...
  if (typeof toolCall.duration === "number" && toolCall.duration > 0)
  ...
  if (
    typeof toolCall.execution_time === "number" &&
    toolCall.execution_time > 0
  )
    return toolCall.execution_time;
  if (
    typeof toolCall.timing?.duration === "number" &&
    toolCall.timing.duration > 0
  )
...
function extractTimingInfo(
  ...
  if (toolCall.startTime && isValidDate(toolCall.startTime)) {
    ...
    toolCall.timing?.startTime &&
    isValidDate(toolCall.timing.startTime)
  ) {
