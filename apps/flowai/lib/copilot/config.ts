...
export function getCopilotConfig(): CopilotConfig {
  ...
  try {
    ...
    const ragMaxTokens = parseIntEnv(
      process.env.COPILOT_RAG_MAX_TOKENS,
      "COPILOT_RAG_MAX_TOKENS"
    );
    ...
    if (process.env.COPILOT_STREAMING_ENABLED) {
      config.general.streamingEnabled =
        process.env.COPILOT_STREAMING_ENABLED === "true";
    }
    ...
    logger.warn(
      "Error applying environment variable overrides, using defaults",
      { error }
    );
  }
...
export function validateCopilotConfig(config: CopilotConfig): {
  ...
  if (
    config.general.maxConversationHistory < 1 ||
    config.general.maxConversationHistory > 50
  ) {
