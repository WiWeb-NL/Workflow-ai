...
const getUserWorkflowTool: CopilotTool = {
  ...
  execute: async (args: Record<string, any>): Promise<CopilotToolResult> => {
    try {
      ...
      const { useWorkflowYamlStore } = await import(
        "@/stores/workflows/yaml/store"
      );
      ...
      const activeWorkflow = activeWorkflowId
        ? registry.workflows[activeWorkflowId]
        : null;
...
export async function executeCopilotTool(
  ...
  try {
    ...
    logger.info(`Copilot tool execution completed: ${toolId}`, {
