import { pgTable, index, foreignKey, text, boolean, integer, timestamp, unique, json, check, uuid, jsonb, vector, uniqueIndex, numeric, pgEnum } from "drizzle-orm/pg-core"
...
export const embedding = pgTable("embedding", {
	...
	index("emb_kb_enabled_idx").using("btree", table.knowledgeBaseId.asc().nullsLast().op("text_ops"), table.enabled.asc().nullsLast().op("bool_ops")),
