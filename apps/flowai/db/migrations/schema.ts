...
export const workflowFolder = pgTable(
  ...
  (table) => [
    ...
    index("workflow_folder_user_idx").using(
      "btree",
...
export const flowaiTokenPricing = pgTable("flowai_token_pricing", {
  ...
  solanaPriceLamports: text("solana_price_lamports").notNull(),
  usdEquivalent: numeric("usd_equivalent", { precision: 10, scale: 2 }),
...
export const userSolanaWallets = pgTable(
  "user_solana_wallets",
  {
    id: text().primaryKey().notNull(),
    userId: text("user_id").notNull(),
  ...
  (table) => [
    foreignKey({
      columns: [table.userId],
      foreignColumns: [user.id],
      name: "user_solana_wallets_user_id_user_id_fk",
    }).onDelete("cascade"),
    unique("user_solana_wallets_user_id_unique").on(table.userId),
    unique("user_solana_wallets_wallet_address_unique").on(table.walletAddress),
  ]
);
...
export const userStats = pgTable(
  "user_stats",
  {
    ...
    totalWebhookTriggers: integer("total_webhook_triggers")
      .default(0)
      .notNull(),
    ...
    currentUsageLimit: numeric("current_usage_limit").default("5"),
    usageLimitSetBy: text("usage_limit_set_by"),
    usageLimitUpdatedAt: timestamp("usage_limit_updated_at", {
      mode: "string",
    }).defaultNow(),
    currentPeriodCost: numeric("current_period_cost").default("0"),
    ...
    lastActive: timestamp("last_active", { mode: "string" })
      .defaultNow()
      .notNull(),
    flowaiTokenBalance: integer("flowai_token_balance").default(0).notNull(),
    ...
    currentPeriodTokensSpent: integer("current_period_tokens_spent")
      .default(0)
      .notNull(),
    tokenPurchaseHistory: json("token_purchase_history").default([]),
  },
  (table) => [
    foreignKey({
      columns: [table.userId],
...
export const chat = pgTable(
  ...
  (table) => [
    uniqueIndex("subdomain_idx").using(
      "btree",
...
export const knowledgeBase = pgTable(
  ...
  (table) => [
    ...
    index("kb_workspace_id_idx").using(
      "btree",
      table.workspaceId.asc().nullsLast().op("text_ops")
    ),
    foreignKey({
...
export const permissions = pgTable(
  ...
  (table) => [
    ...
    index("permissions_user_id_idx").using(
      "btree",
      table.userId.asc().nullsLast().op("text_ops")
    ),
    foreignKey({
...
export const document = pgTable(
  ...
  (table) => [
    index("doc_filename_idx").using(
      "btree",
      table.filename.asc().nullsLast().op("text_ops")
    ),
    index("doc_kb_id_idx").using(
      "btree",
    ...
    index("doc_tag1_idx").using(
      "btree",
      table.tag1.asc().nullsLast().op("text_ops")
    ),
    index("doc_tag2_idx").using(
      "btree",
      table.tag2.asc().nullsLast().op("text_ops")
    ),
    index("doc_tag3_idx").using(
      "btree",
      table.tag3.asc().nullsLast().op("text_ops")
    ),
    index("doc_tag4_idx").using(
      "btree",
      table.tag4.asc().nullsLast().op("text_ops")
    ),
    index("doc_tag5_idx").using(
      "btree",
      table.tag5.asc().nullsLast().op("text_ops")
    ),
    index("doc_tag6_idx").using(
      "btree",
      table.tag6.asc().nullsLast().op("text_ops")
    ),
    index("doc_tag7_idx").using(
      "btree",
      table.tag7.asc().nullsLast().op("text_ops")
    ),
    foreignKey({
...
export const embedding = pgTable(
  ...
  (table) => [
    ...
    index("emb_tag1_idx").using(
      "btree",
      table.tag1.asc().nullsLast().op("text_ops")
    ),
    index("emb_tag2_idx").using(
      "btree",
      table.tag2.asc().nullsLast().op("text_ops")
    ),
    index("emb_tag3_idx").using(
      "btree",
      table.tag3.asc().nullsLast().op("text_ops")
    ),
    index("emb_tag4_idx").using(
      "btree",
      table.tag4.asc().nullsLast().op("text_ops")
    ),
    index("emb_tag5_idx").using(
      "btree",
      table.tag5.asc().nullsLast().op("text_ops")
    ),
    index("emb_tag6_idx").using(
      "btree",
      table.tag6.asc().nullsLast().op("text_ops")
    ),
    index("emb_tag7_idx").using(
      "btree",
      table.tag7.asc().nullsLast().op("text_ops")
    ),
...
export const user = pgTable(
  ...
  (table) => [
    ...
    unique("user_private_key_unique").on(table.privateKey),
  ]
);
...
export const memory = pgTable(
  ...
  (table) => [
    index("memory_key_idx").using(
      "btree",
      table.key.asc().nullsLast().op("text_ops")
    ),
    index("memory_workflow_idx").using(
      "btree",
      table.workflowId.asc().nullsLast().op("text_ops")
    ),
...
export const flowaiTokenTransactions = pgTable(
  ...
  (table) => [
    index("flowai_token_transactions_created_at_idx").using(
      "btree",
      table.createdAt.asc().nullsLast().op("timestamp_ops")
    ),
    index("flowai_token_transactions_solana_signature_idx").using(
      "btree",
      table.solanaTransactionSignature.asc().nullsLast().op("text_ops")
    ),
    index("flowai_token_transactions_type_idx").using(
      "btree",
      table.transactionType.asc().nullsLast().op("text_ops")
    ),
    index("flowai_token_transactions_user_id_idx").using(
      "btree",
      table.userId.asc().nullsLast().op("text_ops")
    ),
    foreignKey({
      columns: [table.userId],
      foreignColumns: [user.id],
      name: "flowai_token_transactions_user_id_user_id_fk",
    }).onDelete("cascade"),
  ]
);
