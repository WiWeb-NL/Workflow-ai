...
const WorkflowContent = React.memo(() => {
  ...
  const [nestedSubflowErrors, setNestedSubflowErrors] = useState<Set<string>>(
    new Set()
  );
  ...
  const { permissions: workspacePermissions, error: permissionsError } =
    useWorkspacePermissions(workspaceId || null);
  ...
  useEffect(() => {
    const handleAddBlockFromToolbar = (event: CustomEvent) => {
      ...
      if (type === "loop" || type === "parallel") {
        ...
        const existingBlocksOfType = Object.values(blocks).filter(
          (b) => b.type === type
        );
        const blockNumber = existingBlocksOfType.length + 1;
        const name =
          type === "loop" ? `Loop ${blockNumber}` : `Parallel ${blockNumber}`;
      ...
      try {
        ...
        if (containerInfo) {
          ...
          if (isAutoConnectEnabled && data.type !== "starter") {
            ...
            if (
              containerType === "loopNode" ||
              containerType === "parallelNode"
            ) {
  ...
  useEffect(() => {
    const validateAndNavigate = async () => {
      ...
      if (!workflows[currentId]) {
        logger.info(
          `Workflow ${currentId} not found, redirecting to first available workflow`
        );
        ...
        if (workspaceWorkflows.length > 0) {
          router.replace(
            `/workspace/${workspaceId}/w/${workspaceWorkflows[0]}`
          );
        } else {
      ...
      if (currentWorkflow && currentWorkflow.workspaceId !== workspaceId) {
        ...
        router.replace(
          `/workspace/${currentWorkflow.workspaceId}/w/${currentId}`
        );
        return;
      }
  ...
  return (
    <div className="flex h-screen w-full flex-col overflow-hidden">
      <div
        className={`${isSidebarCollapsed ? "ml-14" : "ml-60"} transition-all duration-200`}
      >
        <ControlBar hasValidationErrors={nestedSubflowErrors.size > 0} />
      </div>
      <Toolbar />
      ...
      >
        ...
        <ReactFlow
          ...
          onNodeDragStart={
            userPermissions.canEdit ? onNodeDragStart : undefined
          }
          snapToGrid={false}
          snapGrid={[20, 20]}
