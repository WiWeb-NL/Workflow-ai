...
function extractFieldsFromSchema(schema: any): Field[] {
  ...
  return Object.entries(schemaObj.properties).map(
...
export function useBlockConnections(blockId: string) {
  const { edges, blocks } = useWorkflowStore((state) => ({
    edges: state.edges,
    blocks: state.blocks,
  }));
  ...
  const allPathConnections = allPathNodeIds
    .map((sourceId) => {
      ...
      const defaultOutputs: Field[] = Object.entries(
        sourceBlock.outputs || {}
      ).map(([key]) => ({
        name: key,
        type: "string",
  ...
  const directIncomingConnections = edges
    .filter((edge: Edge) => edge.target === blockId)
    .map((edge: Edge) => {
