...
export function useWorkflowExecution() {
      ...
      if (streamContent && result.output && typeof streamContent === "string") {
        ...
        if (enrichedResult.logs) {
          ...
          const streamingBlockId =
            (result.metadata as any)?.streamingBlockId || null;
          ...
          for (const log of enrichedResult.logs) {
            ...
            const isStreamingBlock =
              streamingBlockId && log.blockId === streamingBlockId;
            if (
              isStreamingBlock &&
  ...
  const handleRunWorkflow = useCallback(
    async (workflowInput?: any) => {
      ...
      // Basic authentication check - server-side validation will handle detailed credit checks
      if (!session?.user?.id) {
        addNotification(
          "error",
          "Authentication required to execute workflows",
          activeWorkflowId
        );
        return;
      }
                ...
                try {
                  while (true) {
                    ...
                    if (blockId) {
                      streamedContent.set(
                        blockId,
                        (streamedContent.get(blockId) || "") + chunk
                      );
                    }
                    controller.enqueue(
                      encoder.encode(
            ...
            try {
              ...
              if (result && "success" in result) {
                ...
                if (result.logs) {
                  ...
                  logger.info(
                    `Processed ${processedCount} blocks for streaming tokenization`
                  );
                }

                controller.enqueue(
                  encoder.encode(
                    `data: ${JSON.stringify({ event: "final", data: result })}\n\n`
                  )
                );
  ...
  const executeWorkflow = async (
    ...
    const workflowVars = activeWorkflowId
  ...
  const handleExecutionError = (error: any) => {
    ...
    if (error instanceof Error) {
      ...
      if (
        ...
        errorMessage =
          "API request failed - no specific error details available";
  ...
  const handleStepDebug = useCallback(async () => {
    ...
    try {
      ...
      try {
        addNotification(
          "error",
          `Debug step failed: ${errorMessage}`,
          activeWorkflowId || ""
        );
      } catch (notificationError) {
        logger.error(
          "Error showing step error notification:",
          notificationError
        );
  ...
  const handleResumeDebug = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "Starting resume execution with blocks:",
        currentPendingBlocks
      );
      ...
      while (
        ...
        currentResult = await executor.continueExecution(
          currentPendingBlocks,
          currentContext
        );
      ...
      try {
        ...
        logger.error(
          "Error showing resume error notification:",
          notificationError
        );
