...
export function ControlBar({ hasValidationErrors = false }: ControlBarProps) {
  ...
  const fetchDeployedState = async () => {
    ...
    try {
      ...
      if (requestWorkflowId !== getCurrentActiveWorkflowId()) {
        logger.debug(
          "Workflow changed during deployed state fetch, ignoring response"
        );
        return;
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to fetch deployed state: ${response.statusText}`
        );
      }
      ...
      if (requestWorkflowId === getCurrentActiveWorkflowId()) {
        ...
        logger.debug(
          "Workflow changed after deployed state response, ignoring result"
        );
      }
    } catch (error) {
  ...
  const handleNameClick = () => {
    ...
    setIsEditing(true);
    setEditedName(
      activeWorkflowId ? workflows[activeWorkflowId]?.name || "" : ""
    );
  };
  ...
  const renderAutoLayoutButton = () => {
    ...
    const isDisabled =
      isExecuting || isMultiRunning || isDebugging || !userPermissions.canEdit;
  ...
  const renderRunButton = () => {
    ...
    const isButtonDisabled =
      isWorkflowBlocked || (!canRun && !isLoadingPermissions);
            ...
            <TooltipContent command={getKeyboardShortcutText("Enter", true)}>
              {hasValidationErrors ? (
                <div className="text-center">
                  ...
                  <p className="text-xs">
                    Nested subflows are not supported. Remove subflow blocks
                    from inside other subflow blocks.
                  </p>
                </div>
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs">
                    You've used {usageData?.current?.toFixed(2) || "0.00"}$ of{" "}
                    {usageData?.limit || "0"}$. Upgrade your plan to continue.
                  </p>
                </div>
              ) : !canRun && !isLoadingPermissions ? (
