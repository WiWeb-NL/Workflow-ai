...
export function ChatDeploy({
  ...
  useEffect(() => {
    ...
    if (formRef.current) {
      let deployApiInput = formRef.current.querySelector(
        "#deployApiEnabled"
      ) as HTMLInputElement;
      if (!deployApiInput) {
      ...
      if (response.ok) {
        ...
        if (data.isDeployed && data.deployment) {
          ...
          if (detailResponse.ok) {
            ...
            if (
              chatDetail.authType === "email" &&
              Array.isArray(chatDetail.allowedEmails)
            ) {
            ...
            if (chatDetail.outputConfigs) {
              const configs = Array.isArray(chatDetail.outputConfigs)
                ...
                : [];
              const combinedOutputIds = configs.map(
                (config) => `${config.blockId}_${config.path}`
              );
  ...
  const handleSubdomainChange = (value: string) => {
    ...
    if (
      !lowercaseValue ||
      (originalValues && lowercaseValue === originalValues.subdomain)
    ) {
      return;
    }
      ...
      const majorChanges =
        ...
        (authType === "email" &&
          JSON.stringify(emails) !==
            JSON.stringify(existingChat.allowedEmails));
    ...
    try {
        ...
        const outputConfigs = selectedOutputBlocks
          .map((outputId) => {
            ...
            if (firstUnderscoreIndex !== -1) {
              ...
              logger.warn(
                `Invalid output format: ${outputId}, missing blockId or path`
              );
              return null;
            }
            logger.warn(
      ...
      if (existingChat?.id) {
        ...
        try {
          ...
          logger.warn(
            "Error redeploying API, continuing with chat update:",
            error
          );
        }
      } else {
      ...
      if (!response.ok) {
        throw new Error(
          result.error || `Failed to ${existingChat ? "update" : "deploy"} chat`
        );
      }
      ...
      if (chatUrl) {
        logger.info(
          `Chat ${existingChat ? "updated" : "deployed"} successfully:`,
          chatUrl
        );
      ...
      logger.error(
        `Failed to ${existingChat ? "update" : "deploy"} chat:`,
        error
      );
      setErrorMessage(error.message || "An unexpected error occurred");
      addNotification(
        "error",
        `Failed to deploy chat: ${error.message}`,
        workflowId
      );
    } finally {
      setChatSubmitting(false);
        ...
        <div className="grid gap-4">
          ...
          <div className="space-y-2">
            ...
            <p className="mt-2 text-muted-foreground text-xs">
              Select which block's output to return to the user in the chat
              interface
            </p>
          </div>
                ...
                <Card className="shadow-none">
                  <CardContent className="p-4">
                    <div>
                      ...
                      <p className="text-muted-foreground text-xs">
                        This chat will be publicly accessible to anyone with the
                        link.
                      </p>
                    </div>
                  </CardContent>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You are about to change an active chat deployment. These changes
              will immediately affect all users of your chat.
              {subdomain !== existingChat?.subdomain && (
                <p className="mt-2 font-medium">
                  The URL of your chat will change, and any links to the old URL
                  will stop working.
                </p>
              )}
            </AlertDialogDescription>
