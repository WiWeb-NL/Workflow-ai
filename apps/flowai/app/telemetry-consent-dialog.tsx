...
export function TelemetryConsentDialog() {
  ...
  const telemetryNotifiedUser = useGeneralStore(
    (state) => state.telemetryNotifiedUser
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      logger.error(
        "Error reading telemetry preferences from localStorage:",
        error
      );
    }
  ...
  useEffect(() => {
    ...
    const localStorageNotified =
      typeof window !== "undefined" &&
      localStorage.getItem(TELEMETRY_NOTIFIED_KEY) === "true";
    ...
    if (
      ...
      if (typeof window !== "undefined") {
        try {
          ...
          logger.error(
            "Error saving telemetry notification to localStorage:",
            error
          );
        }
      }
  ...
  const handleAccept = () => {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        logger.error(
          "Error saving telemetry preferences to localStorage:",
          error
        );
      }
    }
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      <AlertDialogContent className="max-w-md">
        <AlertDialogHeader>
          <AlertDialogTitle className="mb-2 font-bold text-2xl">
            Telemetry
          </AlertDialogTitle>
        </AlertDialogHeader>
        ...
        <div className="space-y-4 text-base text-muted-foreground">
          <div>
            To help us improve Visual Workflow AI, we collect anonymous usage
            data by default. This helps us understand which features are most
            useful and identify areas for improvement.
          </div>
