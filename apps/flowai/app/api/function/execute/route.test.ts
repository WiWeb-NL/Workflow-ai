  ...
  describe('Template Variable Resolution', () => {
    ...
    it('should NOT treat email addresses as template variables', async () => {
      const req = createMockRequest('POST', {
        ...
        params: {
          email: {
            from: 'Waleed Latif <waleed@visualworkflowai.ai>',
      ...
      expect(response.status).toBe(200)
      // Should not try to replace <waleed@visualworkflowai.ai> as a template variable
    })
    ...
    it('should handle Gmail webhook data with email addresses containing angle brackets', async () => {
      const gmailData = {
        ...
        rawEmail: {
          ...
          payload: {
            headers: [
              { name: 'From', value: 'Waleed Latif <waleed@visualworkflowai.ai>' },
