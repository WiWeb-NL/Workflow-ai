...
export async function POST(request: NextRequest) {
  try {
    ...
    if (normalizedData) {
      ...
      Object.entries((workflowState.blocks as any) || {}).forEach(
        ([blockId, block]) => {
          subBlockValues[blockId] = {};
          Object.entries((block as any).subBlocks || {}).forEach(
            ([subBlockId, subBlock]) => {
    ...
    const response: any = {
      ...
      note: "This data represents only the blocks and configurations that the user has actually built in their current workflow, not all available Visual Workflow AI capabilities.",
      yaml,
      format: "yaml",
      summary: {
