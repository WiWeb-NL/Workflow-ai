...
async function executeWorkflow(workflow: any, requestId: string, input?: any) {
  ...
  if (usageCheck.isExceeded) {
    logger.warn(
      `[${requestId}] User ${workflow.userId} has exceeded usage limits`,
      {
  ...
  try {
    ...
    logger.debug(
      `[${requestId}] Loading workflow ${workflowId} from normalized tables`
    );
    ...
    if (workflow.variables) {
      try {
        ...
        logger.error(
          `[${requestId}] Failed to parse workflow variables: ${workflowId}`,
          error
        );
    ...
    if (executionResult.success) {
      ...
      console.log(
        `[DEBUG] Attempting to charge FlowAI tokens for successful execution. userId=${workflow.userId}, executionId=${executionId}`
      );
      try {
        const tokenCharged =
...
export async function POST(
  ...
  try {
    ...
    logger.info(
      `[${requestId}] Input passed to workflow:`,
