    ...
    parameters: {
      ...
      properties: Object.entries(toolConfig.params).reduce(
        (acc, [key, config]: [string, any]) => {
          ...
          // Add items property for arrays to satisfy OpenAI function calling schema requirements
          if (config.type === "array") {
            baseProperty.items = {
              type: "string", // Default to string for arrays unless specified otherwise
