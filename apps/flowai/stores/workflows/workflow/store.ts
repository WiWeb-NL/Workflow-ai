    ...
    withHistory((set, get) => ({
      ...
      addBlock: (
        ...
        if (!blockConfig && (type === "loop" || type === "parallel")) {
          ...
          set(newState);
          pushHistory(set as any, get, newState, `Add ${type} node`);
          get().updateLastSaved();
        ...
        set(newState);
        pushHistory(set as any, get, newState, `Add ${type} block`);
        get().updateLastSaved();
        // get().sync.markDirty() // Disabled: Using socket-based sync
      },
      ...
      updateParentId: (id: string, parentId: string, extent: "parent") => {
        ...
        console.log(
          "[WorkflowStore/updateParentId] Updated parentId relationship:",
          {
            blockId: id,
        ...
        pushHistory(
          ...
          newState,
          parentId
            ? `Set parent for ${block.name}`
            : `Remove parent for ${block.name}`
        );
        get().updateLastSaved();
        // Note: Socket.IO handles real-time sync automatically
      },
      ...
      removeBlock: (id: string) => {
        ...
        set(newState);
        pushHistory(set as any, get, newState, "Remove block and children");
        get().updateLastSaved();
        // Note: Socket.IO handles real-time sync automatically
      },
      ...
      addEdge: (edge: Edge) => {
        ...
        set(newState);
        pushHistory(set as any, get, newState, "Add connection");
        get().updateLastSaved();
        // get().sync.markDirty() // Disabled: Using socket-based sync
      },
      ...
      removeEdge: (edgeId: string) => {
        ...
        set(newState);
        pushHistory(set as any, get, newState, "Remove connection");
        get().updateLastSaved();
        // get().sync.markDirty() // Disabled: Using socket-based sync
      },
      ...
      duplicateBlock: (id: string) => {
        ...
        set(newState);
        pushHistory(set as any, get, newState, `Duplicate ${block.type} block`);
        get().updateLastSaved();
        // Note: Socket.IO handles real-time sync automatically
      },
      ...
      updateBlockName: (id: string, name: string) => {
              ...
              function updateReferences(
                ...
                if (Array.isArray(value)) {
                  return value.map((item) =>
                    updateReferences(item, regex, replacement)
                  );
                }
        ...
        set(newState);
        pushHistory(set as any, get, newState, `${name} block name updated`);
        get().updateLastSaved();
        // Note: Socket.IO handles real-time sync automatically
      },
      ...
      revertToDeployedState: async (deployedState: WorkflowState) => {
        ...
        if (
          starterBlock &&
          starterBlock.subBlocks?.startWorkflow?.value === "webhook"
        ) {
          set({ hasActiveWebhook: true });
        }

        pushHistory(set as any, get, newState, "Reverted to deployed state");
        get().updateLastSaved();

        // Call API to persist the revert to normalized tables
        try {
          const response = await fetch(
            `/api/workflows/${activeWorkflowId}/revert-to-deployed`,
            {
              method: "POST",
              headers: {
          ...
          if (!response.ok) {
            ...
            console.error(
              "Failed to persist revert to deployed state:",
              errorData.error
            );
      ...
      toggleBlockAdvancedMode: (id: string) => {
        ...
        if (activeWorkflowId) {
          const subBlockStore = useSubBlockStore.getState();
          const blockValues =
            subBlockStore.workflowValues[activeWorkflowId]?.[id] || {};
      ...
      updateParallelCount: (parallelId: string, count: number) => {
        ...
        set(newState);
        pushHistory(set as any, get, newState, `Update parallel count`);
        get().updateLastSaved();
        // Note: Socket.IO handles real-time sync automatically
      },
      ...
      updateParallelCollection: (parallelId: string, collection: string) => {
        ...
        set(newState);
        pushHistory(set as any, get, newState, `Update parallel collection`);
        get().updateLastSaved();
        // Note: Socket.IO handles real-time sync automatically
      },
      ...
      updateParallelType: (
        ...
        set(newState);
        pushHistory(set as any, get, newState, `Update parallel type`);
        get().updateLastSaved();
        // Note: Socket.IO handles real-time sync automatically
      },
