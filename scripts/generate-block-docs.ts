...
// Paths configuration
const BLOCKS_PATH = path.join(rootDir, "apps/flowai/blocks/blocks");
const DOCS_OUTPUT_PATH = path.join(rootDir, "apps/docs/content/docs/tools");
const ICONS_PATH = path.join(rootDir, "apps/flowai/components/icons.tsx");
...
function extractIcons(): Record<string, string> {
  try {
    ...
    const functionMatches = Array.from(
      iconsContent.matchAll(functionDeclarationRegex)
    );
...
function extractBlockConfig(fileContent: string): BlockConfig | null {
  try {
    ...
    const name =
      extractStringProperty(fileContent, "name") || `${blockName} Block`;
    const description = extractStringProperty(fileContent, "description") || "";
    const longDescription =
      extractStringProperty(fileContent, "longDescription") || "";
...
function findBlockType(content: string, blockName: string): string {
  ...
  const exportMatch = content.match(
    new RegExp(`export\\s+const\\s+${blockName}Block\\s*:`)
  );
  if (exportMatch) {
    // Find the content after the export statement
    const afterExport = content.substring(
...
function extractInputs(content: string): Record<string, any> {
  ...
  propMatches.forEach((propText) => {
    ...
    const _descriptionMatch = propText.match(
      /description\s*:\s*['"]([^'"]+)['"]/s
    );
...
function extractOutputs(content: string): Record<string, any> {
  ...
  if (outputsMatch) {
    ...
    const flatFieldMatches = outputsContent.match(
      /(\w+)\s*:\s*['"]([^'"]+)['"]/g
    );
          ...
          if (typeMatch) {
            ...
            if (propertyMatches) {
              propertyMatches.forEach((propMatch) => {
                ...
                const propParts = propMatch.match(
                  /(\w+)\s*:\s*['"]([^'"]+)['"]/
                );
                if (propParts) {
  ...
  const outputsSection = content.match(
    /outputs\s*:\s*{([^}]*response[^}]*)}(?:\s*,|\s*})/s
  );

  if (outputsSection) {
    // Find the response type definition
    const responseTypeMatch = content.match(
      /response\s*:\s*{\s*type\s*:\s*{([^}]*)}/s
    );
...
function extractToolsAccess(content: string): string[] {
  const accessMatch = content.match(
    /access\s*:\s*\[\s*((?:['"][^'"]+['"](?:\s*,\s*)?)+)\s*\]/
  );
  ...
  try {
    ...
    if (toolConfigMatch) {
      ...
      while ((paramMatch = paramBlocksRegex.exec(paramsContent)) !== null) {
        ...
        let descriptionMatch = paramBlock.match(
          /description\s*:\s*['"]([^'"]+)['"]/
        );
        if (!descriptionMatch) {
    ...
    const outputRegex =
      /transformResponse[\s\S]*?return\s*{[\s\S]*?output\s*:\s*{([^}]*)/;
...
async function getToolInfo(toolName: string): Promise<{
  ...
  try {
    ...
    for (let i = parts.length - 1; i >= 1; i--) {
      ...
      if (
        fs.existsSync(toolDirPath) &&
        fs.statSync(toolDirPath).isDirectory()
      ) {
...
function mergeWithManualContent(
  ...
  Object.keys(manualSections).forEach((section) => {
    console.log(
      `  - ${section}: ${manualSections[section].substring(0, 20)}...`
    );
  });
  ...
  Object.entries(manualSections).forEach(([sectionName, content]) => {
    ...
    if (insertionPoint) {
      ...
      if (match && match.index !== undefined) {
        ...
        console.log(
          `Inserting ${sectionName} content after position ${insertPosition}`
        );
...
async function generateBlockDoc(
  blockPath: string,
  ...
  try {
    ...
    if (fs.existsSync(outputFilePath)) {
      ...
      console.log(
        `Existing file found for ${blockConfig.type}.mdx, checking for manual content...`
      );
    }

    // Extract manual content from existing file before generating new content
    const manualSections = existingContent
      ? extractManualContent(existingContent)
      : {};
    ...
    if (Object.keys(manualSections).length > 0) {
      console.log(
        `Found manual content in ${blockConfig.type}.mdx, merging...`
      );
      finalContent = mergeWithManualContent(
        markdown,
        existingContent,
        manualSections
      );
    ...
    for (const tool of tools.access) {
      ...
      if (toolInfo) {
        if (
          ...
          toolInfo.description !== "No description available"
        ) {
