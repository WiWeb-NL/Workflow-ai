...
program
  .option(
    "-p, --port <port>",
    "Port to run Visual Workflow AI on",
    DEFAULT_PORT
  )
...
async function ensureNetworkExists(): Promise<boolean> {
  try {
    ...
    if (!networks.includes(NETWORK_NAME)) {
      console.log(
        chalk.blue(`🔄 Creating Docker network '${NETWORK_NAME}'...`)
      );
...
async function main() {
  ...
  if (!existsSync(dataDir)) {
    try {
      ...
      console.error(
        chalk.red(`❌ Failed to create data directory: ${dataDir}`)
      );
      process.exit(1);
    }
  }
  ...
  console.log(
    chalk.green(
      `✅ Visual Workflow AI is now running at ${chalk.bold(`http://localhost:${port}`)}`
    )
  );
  console.log(
    chalk.yellow(
